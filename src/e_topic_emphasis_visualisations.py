...
def _selection_title(selection: Optional[DataSelectionConfig]) -> str:
    ...
    if genres and set(genres) != set(TXT_AUTHOR_DIRS):
        label = "Author" if len(genres) == 1 else "Authors"
...
def _ordered_genres(genres: Iterable[str]) -> List[str]:
    ...
    ordered = [genre for genre in TXT_AUTHOR_DIRS if genre in genre_set]
...
def plot_central_topic_x_bars(
    ...
    *,
    ...
    ordered_genres = [genre for genre in TXT_AUTHOR_DIRS if genre in genre_set]
...
def plot_central_topic_x_bars_stability_all(
    ...
    *,
    ...
    title = (
        f"Central Topic to Metric Correlations (Stable {stability_config.metric_key} {comp} "
        f"{stability_config.threshold:g}) - All authors"
    )
...
def plot_presence_comparison_panels(
    *,
    ...
    central_path = _dashboard_report_path(base_dir, text_name, CENTRAL_TOPIC_PRESENCE_CORRELATIONS_SUFFIX)
    all_path = _dashboard_report_path(base_dir, text_name, TOPIC_PRESENCE_CORRELATIONS_SUFFIX)
...
def plot_topic_variance_timeline_panel(
    *,
    ...
    root = dashboard_root or results_path("dashboard", block_size=DEFAULT_BLOCK_SIZE)
    base_dir = root / genre / author / text_name
    presence_path = _dashboard_report_path(base_dir, text_name, CENTRAL_TOPIC_PRESENCE_CORRELATIONS_SUFFIX)
...
def plot_topic_centrality_quadrant(
    *,
    ...
    root = dashboard_root or results_path("dashboard", block_size=DEFAULT_BLOCK_SIZE)
    base_dir = root / genre / author / text_name
    topic_path = _dashboard_report_path(base_dir, text_name, TOPIC_CORRELATIONS_SUFFIX)
...
def plot_aggregated_presence_heatmap(
    *,
    ...
    genres = [genre for genre in TXT_AUTHOR_DIRS if not selection or _matches_selection(genre, "", "", selection)]
...
def _window_metrics_path(genre: str, author: str, text_name: str) -> Path:
    return window_metrics_path(genre=genre, author=author, text_name=text_name, domain="syntax")


def _dashboard_report_path(base_dir: Path, text_name: str, suffix: str) -> Path:
...
def generate_all_visualisations_with_config(
    *,
    ...
    for block_size in block_sizes:
        dashboard_root = block_results_path(
            "dashboard",
            block_size,
