...
from src.e0_dashboard_metrics import (
...
def _write_text_central_topic_scores(
    ...
    *,
    ...
    if not entries:
        return
    output_path = analytics_path("dashboard", filename=TEXT_CENTRAL_TOPIC_SCORES_FILENAME)
...
def _write_genre_topic_scores(
    ...
    *,
    ...
    for genre, genre_entries in _group_by_genre(entries).items():
        output_path = analytics_path("dashboard", genre, filename=GENRE_TOPIC_SCORES_FILENAME)
        if use_existing and output_path.exists():
            continue
...
def _write_genre_metric_variances(
    ...
    *,
    ...
    if not entries:
        return
    for genre, genre_entries in _group_by_genre(entries).items():
        output_path = analytics_path("dashboard", genre, filename=GENRE_METRIC_VARIANCE_FILENAME)
        if use_existing and output_path.exists():
            continue
...
def _write_genre_central_presence_correlations(
    ...
    *,
    ...
    if not entries:
        return
    for genre, genre_entries in _group_by_genre(entries).items():
        output_path = analytics_path("dashboard", genre, filename=GENRE_CENTRAL_PRESENCE_FILENAME)
        if use_existing and output_path.exists():
            continue
...
def run_dashboard(
    ...
    *,
    ...
    Outputs (aggregates):
      data/analytics/dashboard/<genre>/00_genre_central_topic_presence_correlations.json
    """
