...
def run_dashboard(
    ...
    *,
    ...
    for file in tqdm(window_metric_files, desc="Dashboard correlations", ascii=True):
        ...
        topic_presence_file = out_dir / f"{text_name}{TOPIC_PRESENCE_CORRELATIONS_SUFFIX}"
        non_central_presence_file = (
            out_dir / f"{text_name}{NON_CENTRAL_TOPIC_PRESENCE_CORRELATIONS_SUFFIX}"
        )
        variance_file = out_dir / f"{text_name}{WINDOW_VARIANCE_SUFFIX}"
        topic_entry = None
        ...
        if use_existing and topic_presence_file.exists():
            try:
                with open(topic_presence_file, "r", encoding="utf-8") as f:
        ...
        if use_existing and non_central_presence_file.exists():
            try:
                with open(non_central_presence_file, "r", encoding="utf-8") as f:
        ...
        variance_entry = None
        if use_existing and variance_file.exists():
            try:
